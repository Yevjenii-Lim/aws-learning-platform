...
import { dynamoClient, TABLES, DynamoService, DynamoTutorial } from '../lib/dynamodb';
import { PutCommand } from '@aws-sdk/lib-dynamodb';
import { uploadLessonContent, getLessonKey, getServiceKey } from '../lib/s3';
...
async function migrateToAWS() {
  ...
  try {
    ...
    for (const service of awsServices) {
      ...
      const serviceData: DynamoService = {
      ...
      for (const tutorial of service.tutorials) {
        ...
        const tutorialData: DynamoTutorial = {
          PK: `SERVICE#${service.id}`,
